				...
				bool is_gapless_mode)
{
	...
	pr_debug("%s: session[%d] wr_format[0x%x] bits_per_sample %d stream_id %d, ac->io_mode=%d",
		...
		uint32_t msw_ts, uint32_t lsw_ts, uint32_t stream_id)
{
	...
	pr_debug("session[%d] stream %d", ac->session, stream_id);
...
static int __q6asm_cmd(struct audio_client *ac, int cmd, uint32_t stream_id)
{
	...
	case CMD_OUT_FLUSH:
		...
		break;
	case CMD_EOS:
		pr_debug("%s:q6asm session %d send EOS \n", __func__,ac->session);
		hdr.opcode = ASM_DATA_CMD_EOS;
		atomic_set(&ac->cmd_state, 0);
		state = &ac->cmd_state;
		break;
	case CMD_CLOSE:
		pr_debug("%s:q6asm close session %d\n", __func__,ac->session);
	...
	pr_debug("%s:session[%d]opcode[0x%x] stream %d", __func__,
						ac->session,
...
int q6asm_get_apr_service_id(int session_id)
{
	...
	if (session[session_id] == NULL || ((struct apr_svc *)session[session_id]->apr) == NULL) {
		pr_err("%s: could not get audio client for session_id = %d", __func__, session_id);
		return -EINVAL;
	}
