...
void send_abnormal_usb_reset_uevent(struct work_struct *work)
{
	...
	if (!ret)
		pr_debug("broadcast abnormal usb reset\n");
	else
		pr_debug("fail to broadcast abnormal usb reset\n");
}
