...
int32_t vd6869_read_otp_valid_layer(struct msm_sensor_ctrl_t *s_ctrl, int8_t *valid_layer,
		bool first)
{
	int32_t rc = 0;
...
int32_t vd6869_read_otp_memory(struct sensorb_cfg_data *cdata, struct msm_sensor_ctrl_t *s_ctrl,
		bool first)
{
	int32_t rc = 0;
	const int32_t offset = OTP_BUFFER_OFFSET;
	...
	uint8_t *path= "/data/vendor/camera/OTPData.dat";
	struct file* f;
...
static int vd6869_read_module_vendor(struct msm_sensor_ctrl_t *s_ctrl,
		uint8_t valid_layer, uint8_t* module_vendor, uint8_t* driver_ic, bool first)
{
	int32_t rc = 0;
	uint16_t read_data = 0;
		...
		struct msm_sensor_ctrl_t *s_ctrl, bool first, uint8_t valid_layer)
{
	...
	pr_info("%s: ModuleFuseID= %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\n", __func__,
			otp[0x12], otp[0x13], otp[0x14], otp[0x15], otp[0x16], otp[0x17],
			otp[0x18], otp[0x19], otp[0x1a], otp[0x1b], otp[0x1c], otp[0x1d]);
